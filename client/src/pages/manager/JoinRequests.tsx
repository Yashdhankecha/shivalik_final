...
const ManagerJoinRequests = () => {
  ...
  const handleReject = async (requestId: string) => {
    ...
    try {
      await managerApi.rejectCommunityJoinRequest(communityId!, requestId, rejectionReason);
      toast({
        title: "Success",
      ...
      <div className="mb-6">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
          <div>
            ...
            <p className="text-gray-600">Manage user requests to join your community</p>
          </div>
        </div>
                ...
                {requests.length === 0 ? (
                  <tr>
                    <td colSpan={5} className="px-6 py-8 text-center text-gray-500">
                      No join requests found. {pagination.total === 0 ? 'There are no pending join requests for your community.' : 'Try adjusting your search criteria.'}
                    </td>
                  </tr>
                ) : (
